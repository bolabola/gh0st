...
LPBYTE CSystemManager::getProcessList()
{
    ...
    //TODO: 这种遍历进程列表的逻辑丢掉了太多的进程，要改掉！by zhangyl
	...
	{	  
		...
		{      
			...
			if ((pe32.th32ProcessID !=0 ) && (pe32.th32ProcessID != 4) && (pe32.th32ProcessID != 8))
			{
				...
				if (!GetModuleFileNameEx(hProcess, hModules, strProcessName, ARRAYSIZE(strProcessName)))
                    continue;
				
				// 此进程占用数据大小
				dwLength = sizeof(DWORD) + lstrlen(pe32.szExeFile) * sizeof(TCHAR) + lstrlen(strProcessName) * sizeof(TCHAR) + 2 * sizeof(TCHAR);
				// 缓冲区太小，再重新分配下
				...
				memcpy(lpBuffer + dwOffset, pe32.szExeFile, (lstrlen(pe32.szExeFile) + 1) * sizeof(TCHAR));
                dwOffset += (lstrlen(pe32.szExeFile) + 1) * sizeof(TCHAR);
				
				memcpy(lpBuffer + dwOffset, strProcessName, (lstrlen(strProcessName) + 1) * sizeof(TCHAR));
...
bool CSystemManager::DebugPrivilege(const TCHAR *PName, BOOL bEnable)
{
	BOOL              bResult = TRUE;
	HANDLE            hToken;
	...
	BOOL bRet = AdjustTokenPrivileges(hToken, FALSE, &TokenPrivileges, sizeof(TOKEN_PRIVILEGES), NULL, NULL);
    //TODO: 存在部分权限拿不到
...
bool CALLBACK CSystemManager::EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
    ...
    GetWindowText(hwnd, strTitle, ARRAYSIZE(strTitle));
	...
	memcpy(lpBuffer + dwOffset + sizeof(DWORD), strTitle, (lstrlen(strTitle) + 1) * sizeof(TCHAR));
