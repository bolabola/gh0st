...
BOOL CCJListView::SortIntItems( int nCol, BOOL bAscending, int low /*= 0*/, int high /*= -1*/ )
{
	...
	midItem = _wtoi(GetListCtrl().GetItemText( (lo+hi)/2, nCol ).GetBuffer(0));
	...
	while( lo <= hi )
	{
		...
		// the partition element starting from the left Index.
		if( bAscending ) {
            while ((lo < high) && (_wtoi(GetListCtrl().GetItemText(lo, nCol).GetBuffer(0)) < midItem))
				++lo;
		}
		else {
            while ((lo < high) && (_wtoi(GetListCtrl().GetItemText(lo, nCol).GetBuffer(0)) > midItem))
		...
		// the partition element starting from the right Index.
		if( bAscending ) {
            while ((hi > low) && (_wtoi(GetListCtrl().GetItemText(hi, nCol).GetBuffer(0)) > midItem))
				--hi;
		}
		...
		{
			// swap only if the items are not equal
            if (_wtoi(GetListCtrl().GetItemText(lo, nCol).GetBuffer(0)) != _wtoi(GetListCtrl().GetItemText(hi, nCol).GetBuffer(0)))
			{
				// swap the rows
				LV_ITEM lvitemlo, lvitemhi;
				int nColCount =
