...
private:
	...
	bool MakeSureDirectoryPathExists(LPCSTR pszDirPath);
	bool UploadToRemote(LPBYTE lpBuffer);
	bool FixedUploadList(LPCSTR lpszDirectory);
	void StopTransfer();
	UINT SendFilesList(LPCSTR lpszDirectory);
	bool DeleteDirectory(LPCSTR lpszDirectory);
	UINT SendFileSize(LPCSTR lpszFileName);
