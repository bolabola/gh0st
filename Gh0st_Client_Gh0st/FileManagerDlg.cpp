	...
	: CDialog(CFileManagerDlg::IDD, pParent)
{
    ...
    //标志位占2个字节，所以偏移量是2
	memcpy(m_bRemoteDriveList, m_pContext->m_DeCompressionBuffer.GetBuffer(2), m_pContext->m_DeCompressionBuffer.GetBufferLen() - 2);
...
void CFileManagerDlg::FixedLocalFileList(CString directory)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{	
            ...
            wsprintf(szIconIndex, _T("local: %s-%d\n"), file.GetFileName(), GetIconIndex(file.GetFileName(), GetFileAttributes(file.GetFilePath())));
...
void CFileManagerDlg::FixedRemoteDriveList()
{
	...
	TCHAR	*pDrive = NULL;
    pDrive = (TCHAR *)m_bRemoteDriveList;
...
void CFileManagerDlg::GetRemoteFileList(CString directory)
{
    ...
    memcpy(bPacket + 1, m_Remote_Path.GetBuffer(0), PacketSize - 1 - sizeof(TCHAR));
...
void CFileManagerDlg::FixedRemoteFileList(BYTE *pbBuffer, DWORD dwBufferLen)
{
	...
	*/
	...
	if (dwBufferLen != 0)
	{
		...
		{
			// 跳过Token，共2字节
            TCHAR *pList = (TCHAR *)(pbBuffer + 2);
            for (TCHAR *pBase = pList; (pList - pBase) * 2 < dwBufferLen - 1;)
			{
				...
				if (bIsInsert)
				{
                    memcpy(&iconIndex, pList + lstrlen(pszFileName) + 1, sizeof(int));

                    TCHAR szIconIndex[128] = { 0 };
                    wsprintf(szIconIndex, _T("%s-%d\n"), pszFileName, iconIndex);
                    ::OutputDebugString(szIconIndex);

                    //TODO: 这个问题待后续解决
                    //nItem = m_list_remote.InsertItem(/*nItemIndex++*/0, pszFileName, /*GetIconIndex(pszFileName, nType)*/iconIndex);
                    nItem = m_list_remote.InsertItem(nItemIndex++, pszFileName);
				...
				// 得到文件大小
				pList += lstrlen(pszFileName) + 1 + 2;
				if (bIsInsert)
				{
					memcpy(&dwFileSizeHigh, pList, sizeof(DWORD));
                    memcpy(&dwFileSizeLow, pList + 2, sizeof(DWORD));
					CString strSize;
					strSize.Format(_T("%10d KB"), (dwFileSizeHigh * (MAXDWORD+1)) / 1024 + dwFileSizeLow / 1024 + (dwFileSizeLow % 1024 ? 1 : 0));
					m_list_remote.SetItemText(nItem, 1, strSize);
					memcpy(&ftm_strReceiveLocalFileTime, pList + 4, sizeof(FILETIME));
					CTime	time(ftm_strReceiveLocalFileTime);
					m_list_remote.SetItemText(nItem, 3, time.Format("%Y-%m-%d %H:%M"));	
				}

                //跳过8个TCHAR的大小，这8个TCHAR分别是dwFileSizeHigh、dwFileSizeLow和FILETIME大小之和
				pList += 8;
			}
		}
	}
...
void CFileManagerDlg::WriteLocalRecvFile()
{
	...
	{
		::MessageBox(m_hWnd, m_strReceiveLocalFile + _T(" 文件写入失败"), __T("警告"), MB_OK|MB_ICONWARNING);
	}
	CloseHandle(hFile);
	// 为了比较，计数器递增
...
void CFileManagerDlg::OnLocalDelete()
{
	...
	if (::MessageBox(m_hWnd, str, __T("确认删除"), MB_YESNO|MB_ICONQUESTION) == IDNO)
		return;
