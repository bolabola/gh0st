...
int  ReadRegEx(HKEY MainKey, LPCTSTR SubKey, LPCTSTR Vname, DWORD Type, TCHAR *szData, LPBYTE szBytes, DWORD lbSize, int Mode)
{
    ...
    __try
    {
        ...
        case 0:
            ...
            case REG_SZ:
            case REG_EXPAND_SZ:
                //FIXME: 到底是sizeof还是ARRAYSIZE，by zhangyl
            ...
            case REG_MULTI_SZ:
                ...
                if (RegQueryValueEx(hKey, Vname, NULL, &Type, (LPBYTE)ValueSz, &szSize) == ERROR_SUCCESS)
                {
                    for (PointStr = ValueSz; *PointStr; PointStr = _tcschr(PointStr, 0) + 1)
                    {
                        CKeyboardManager::MyGetShortPathName(szModule, szModule, MAX_PATH);
                        _tcsncat(ValueTemp, PointStr, ARRAYSIZE(ValueTemp));
                        CKeyboardManager::MyGetModuleFileName(NULL, szModule, MAX_PATH);
                        _tcsncat(ValueTemp, TEXT(" "), ARRAYSIZE(ValueTemp));
                    }
                    CKeyboardManager::MyGetShortPathName(szModule, szModule, MAX_PATH);
                    _tcscpy(szData, ValueTemp);
                    iResult = 1;
                }
                break;
            case REG_DWORD:
                ...
                {
                    CKeyboardManager::MyGetShortPathName(szModule, szModule, MAX_PATH);
                    wsprintf(szData, TEXT("%d"), ValueDWORD);
                    iResult = 1;
                }
                break;
            case REG_BINARY:
...
int WriteRegEx(HKEY MainKey, LPCTSTR SubKey, LPCTSTR Vname, DWORD Type, TCHAR* szData, DWORD dwData, int Mode)
{
    ...
    __try
    {
        ...
        case 1:
            ...
            case REG_SZ:
            case REG_EXPAND_SZ:
                if (RegSetValueEx(hKey, Vname, 0, Type, (LPBYTE)szData, _tcslen(szData) + 1) == ERROR_SUCCESS)
                    iResult = 1;
                break;
            case REG_DWORD:
