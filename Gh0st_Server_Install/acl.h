...
{
BOOL result;
typedef BOOL (WINAPI *lpAddFun)(PSECURITY_DESCRIPTOR,PSECURITY_DESCRIPTOR_CONTROL,LPDWORD);		//返回值,形参类型需要参考MSDN
HINSTANCE hDll=LoadLibrary("kernel32.dll");			//函数所在的DLL
lpAddFun addFun=(lpAddFun)GetProcAddress(hDll,"GetSecurityDescriptorControl");	//函数名字
if (addFun != NULL)
	{
	addFun(pSecurityDescriptor,pControl,lpdwRevision);					//调用函数
...
{
BOOL result;
typedef BOOL (WINAPI *lpAddFun)(PACL,DWORD,DWORD,LPVOID,DWORD);//返回值,形参类型需要参考MSDN
HINSTANCE hDll=LoadLibrary("advapi32.dll");//函数所在的DLL
lpAddFun addFun=(lpAddFun)GetProcAddress(hDll,"AddAce");//函数名字
if (addFun != NULL)
	{
	addFun(pAcl,dwAceRevision,dwStartingAceIndex,pAceList,nAceListLength);//调用函数
	FreeLibrary(hDll);//释放句柄
	}
return result;
}
...
{
HMODULE result;
typedef HMODULE (WINAPI *lpAddFun)(LPCSTR);//返回值,形参类型需要参考MSDN
HINSTANCE hDll=LoadLibrary("kernel32.dll");//函数所在的DLL
lpAddFun addFun=(lpAddFun)GetProcAddress(hDll,"GetModuleHandleA");//函数名字
if (addFun != NULL)
	{
	addFun( lpModuleName);//调用函数
	FreeLibrary(hDll);//释放句柄
	}
return result;
}
